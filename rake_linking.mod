...
param T_RL{RAKELINKS}; # Total time required for rake link j (in minutes)
...
# Haul time constraint: (Service 2 and 3 must fit within halt time if linked)
subject to HaulTimeConstraint {j in RAKELINKS}:
    (T_a[3] - T_d[2]) * (X[2,j] + X[3,j] - 1) <= e1 * T_h;

    
subject to Commercial_Loss_Constraint {k in SERVICES, kp in SERVICES, j in RAKELINKS:
    C[k] < C[kp]}:
    M2*(1-X[k,j]) + C[k] * X[k,j] >= 0.9 * C[kp] * X[kp,j];
